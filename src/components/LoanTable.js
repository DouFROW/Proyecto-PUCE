    ...
    <TableContainer sx={{ boxShadow: 'none' }}>
      <Table size="small">
        <TableHead>
          <TableRow>
            {headers.map((header) => (
              <TableCell key={header} sx={{ fontWeight: 'medium', color: '#666', fontSize: '0.875rem', padding: '8px 16px' }}>
                {header}
              </TableCell>
            ))}
          </TableRow>
            ...
            <TableRow key={index} hover>
              ...
              <TableCell sx={{ padding: '8px 16px' }}>
                <Chip 
                  ...
                  size="small" 
                  sx={{ 
                    backgroundColor: row.Estado === 'Aprobado' ? '#28a745' : 
                                    row.Estado === 'Pendiente' ? '#ffc107' : '#dc3545',
